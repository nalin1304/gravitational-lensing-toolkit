...
class GeodesicIntegrator:
    ...
    def integrate_deflection(
        ...
        term2 = L**2 / r_init**2
        dr_dlambda_sq = E**2 - metric_factor * (
            term2 + 0
        )  # Last term is for massive particles

        if dr_dlambda_sq < 0:
            raise ValueError(
                f"Geodesic cannot reach r={r_init / self.rs:.2f}rs with b={b_over_rs:.2f}rs. "
                "Photon captured by black hole or invalid initial conditions. "
                "This is a physical impossibility, not a numerical error. "
        ...
        alpha_exact = self._integrate_schwarzschild_orbit(r_init, b, lambda_steps)
        ...
        # Relative error
        relative_error = (
            abs(alpha_exact - alpha_simplified) / alpha_exact if alpha_exact != 0 else 0
        )
    ...
    def _integrate_schwarzschild_orbit(
        self, r_init: float, b: float, steps: int
    ) -> float:
        """
    ...
    def compare_strong_vs_weak_field(
        self, b_min_rs: float = 1.5, b_max_rs: float = 100, n_points: int = 20
    ) -> Dict:
        """
        ...
        mean_error_strong = (
            np.mean(rel_errors[strong_mask]) if np.any(strong_mask) else 0
        )
...
def validate_paper_accuracy_table(mass: float = 1e12) -> Dict:
    ...
    return {
        ...
        "validation_passed": all(
            0.4 < row["error"] < 0.6 for row in table_rows
        ),  # ~50% error
    }


# Convenience function for quick testing
def quick_deflection_comparison(mass: float, impact_parameter_rs: float) -> None:
    """
...
if __name__ == "__main__":
    ...
    if EINSTEINPY_AVAILABLE:
        ...
        print("=" * 70)
        print(
            f"{'b/rs':>8} | {'Simplified α':>12} | {'GR α':>12} | {'Error':>8} | {'Regime':>15}"
        )
        print("-" * 70)
