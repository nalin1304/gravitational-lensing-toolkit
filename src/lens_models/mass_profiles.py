...
class NFWProfile(MassProfile):
    ...
    def __init__(
        ...
        # Axis ratio b/a = (1-e)/(1+e) - matches advanced_profiles.py convention
        self.q = (1.0 - self.ellipticity) / (1.0 + self.ellipticity)
    ...
    def _compute_nfw_parameters(self):
        ...
        delta_c = (
            (Delta_vir / 3) * (self.c**3) / (np.log(1 + self.c) - self.c / (1 + self.c))
        )
        ...
        if self._apply_inner_slope_correction and z_lens > 0.4:
            ...
            self._inner_slope_correction = gamma_inner
            self._inner_slope_redshift_applied = True

            # Apply correction to characteristic density
            # ρ_s_corrected = ρ_s × (γ_in / γ_NFW)
            # This accounts for the shallower inner profile at high z
            self.rho_s *= gamma_inner
            self.rho_s_msun_kpc3 *= gamma_inner

            # Recompute kappa_s with corrected density
            rho_s_msun_pc3_corrected = self.rho_s / 1e18
            Sigma_s_corrected = rho_s_msun_pc3_corrected * r_s_pc
            self.kappa_s = Sigma_s_corrected / self.sigma_crit
        else:
            self._inner_slope_correction = 1.0
            self._inner_slope_redshift_applied = False
    ...
    def _generate_subhalos(self):
        ...
        if alpha != 1.0:
            M_sub = (
                (M_max ** (1 - alpha) - M_min ** (1 - alpha)) * u + M_min ** (1 - alpha)
            ) ** (1 / (1 - alpha))
        else:
        ...
        for i, (M_i, (x_i, y_i)) in enumerate(zip(M_sub, positions)):
            ...
            subhalo = NFWProfile(
                M_i, c_sub, self.lens_system, ellipticity=0.0, include_subhalos=False
            )
    ...
    def _f_nfw(self, x: np.ndarray) -> np.ndarray:
        ...
        if np.any(mask2):
            ...
            f[mask2] = (1 / (x2**2 - 1)) * (
                1 - (2 / np.sqrt(x2**2 - 1)) * np.arctan(np.sqrt((x2 - 1) / (x2 + 1)))
            )
    ...
    def _g_nfw_convergence(self, x: np.ndarray) -> np.ndarray:
        """
        NFW convergence function g(x) - DIFFERENT from deflection function f(x)!

        This is used to compute κ(r) = 2κ_s × g(r/r_s)

        Reference: Wright & Brainerd 2000, Eq. 10-13
        The convergence is the surface mass density normalized by critical density:
        κ(x) = Σ(x) / Σ_crit
    ...
    def deflection_angle(
        ...
        # Calculate f(x) (deflection function - Wright & Brainerd 2000)
        f_vals = self._f_nfw(x_scaled)

        # Deflection angle magnitude: α(r) = 4 κ_s r_s × f(x) / x
        # where x = r/r_s (CORRECT formula)
    ...
    def convergence(
        ...
        # FIXED: κ(r) = 2 κ_s × g(x) where g(x) is the convergence function
        # This is DIFFERENT from the deflection function f(x)!
        # Reference: Wright & Brainerd 2000, Bartelmann 1996
        g_vals = self._g_nfw_convergence(x_scaled)
    ...
    def lensing_potential(
        ...
        Uses the proper analytical formula from Wright & Brainerd (2000):
        ψ(x) = 2κ_s r_s² × h(x)

        where x = r/r_s and h(x) is:
        - For x < 1: h(x) = ln(x/2)² - arctanh²(√(1-x²))
        - For x = 1: h(x) = ln(1/2)²
        - For x > 1: h(x) = ln(x/2)² + arctan²(√(x²-1))
        ...
        # Compute h(x) function for lensing potential
        h_vals = np.zeros_like(x_scaled, dtype=float)

        # Case x < 1 (inner region)
        mask1 = x_scaled < 1
        if np.any(mask1):
            x1 = x_scaled[mask1]
            sqrt_term = np.sqrt(np.maximum(1 - x1**2, 1e-10))
            # h(x) = ln(x/2)² - arctanh(√(1-x²))²
            # Note: arctanh(z) = 0.5 * ln((1+z)/(1-z))
            term1 = np.log(x1 / 2.0) ** 2
            # Use safe arctanh to avoid numerical issues
            arg_atanh = np.clip(sqrt_term, 0, 0.999999)
            term2 = np.arctanh(arg_atanh) ** 2
            h_vals[mask1] = term1 - term2

        # Case x > 1 (outer region)
        mask2 = x_scaled > 1
        if np.any(mask2):
            x2 = x_scaled[mask2]
            sqrt_term = np.sqrt(np.maximum(x2**2 - 1, 1e-10))
            # h(x) = ln(x/2)² + arctan(√(x²-1))²
            term1 = np.log(x2 / 2.0) ** 2
            term2 = np.arctan(sqrt_term) ** 2
            h_vals[mask2] = term1 + term2

        # Case x = 1
        mask3 = np.isclose(x_scaled, 1.0, rtol=1e-5)
        if np.any(mask3):
            h_vals[mask3] = np.log(0.5) ** 2  # ln(1/2)²

        # Full potential: ψ = 2 × κ_s × r_s² × h(x)
        psi = 2.0 * self.kappa_s * (self.r_s**2) * h_vals
...
class WarmDarkMatterProfile(NFWProfile):
    ...
    def __init__(
        self, M_vir: float, concentration: float, lens_system, m_wdm: float = np.inf
    ):
    ...
    def __repr__(self):
        ...
        if np.isfinite(self.m_wdm):
            return (
                f"WarmDarkMatterProfile(M_vir={self.M_vir:.2e} Msun, "
...
class SIDMProfile(NFWProfile):
    ...
    def __init__(
        self, M_vir: float, concentration: float, lens_system, sigma_SIDM: float = 0.0
    ):
    ...
    def __repr__(self):
        ...
        if self.sigma_SIDM > 0:
            return (
                f"SIDMProfile(M_vir={self.M_vir:.2e} Msun, c={self.c:.2f}, "
...
class DarkMatterFactory:
    ...
    @staticmethod
    def create_halo(
        model_type: str, M_vir: float, concentration: float, lens_system, **kwargs
    ) -> MassProfile:
        """
        ...
        **kwargs : dict
        ...
        if model_type == "CDM":
            ...
            raise ValueError(
                f"Unknown model type: {model_type}. Supported: 'CDM', 'WDM', 'SIDM'"
            )

    @staticmethod
    def generate_random_halo(
        model_type: str,
        lens_system,
    ...
    def validate_mass_conservation(
        halo: MassProfile, r_max: float = None, tolerance: float = 0.1
    ) -> Dict:
        """
        ...
        M_integrated = trapezoid(integrand, r_grid)  # Msun
        ...
        fractional_error = (
            abs(M_integrated - M_expected) / M_expected if M_expected != 0 else 0.0
        )
