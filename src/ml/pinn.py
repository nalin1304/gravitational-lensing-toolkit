...
class PhysicsInformedNN(nn.Module):
    ...
    def __init__(self, input_size: int = 64, dropout_rate: float = 0.2):
        ...
        self.param_head = nn.Sequential(
            nn.Linear(256, 128), nn.ReLU(), nn.Dropout(dropout_rate), nn.Linear(128, 5)
        )

        # Classification head for DM model type
        # Output: [p_CDM, p_WDM, p_SIDM]
        self.class_head = nn.Sequential(
            nn.Linear(256, 128), nn.ReLU(), nn.Dropout(dropout_rate), nn.Linear(128, 3)
        )
...
def compute_nfw_deflection(
    ...
    kpc_to_km = 3.086e16  # 1 kpc in km
    ...
    # FIXED: Use proper angular diameter distance formula instead of linear approximation
    # The linear approximation D ≈ c*z/H0 is only valid for z << 1
    # For gravitational lensing with z_l ~ 0.5, z_s ~ 2.0, we need proper calculation

    def angular_diameter_distance(
        z: float, H0: float = 70.0, Omega_m: float = 0.3
    ) -> float:
        """
        Calculate proper angular diameter distance for flat ΛCDM.

        D(z) = (c/H0) * (1+z)⁻¹ * ∫₀^z dz' / E(z')
        ...
        """
        # Use astropy if available for accuracy
        try:
        ...
        def E(z_val):
            return np.sqrt(Omega_m * (1 + z_val) ** 3 + (1 - Omega_m))

        # Numerical integration
        result, _ = integrate.quad(lambda zp: 1.0 / E(zp), 0, z)

        c_km_s = 299792.458  # km/s
        D = (c_km_s / H0) * (1 + z) ** -1 * result
        return D

    # Compute proper angular diameter distances
    if isinstance(z_l, torch.Tensor):
        z_l_np = z_l.item() if z_l.numel() == 1 else z_l[0].item()
    else:
        z_l_np = z_l

    if isinstance(z_s, torch.Tensor):
        z_s_np = z_s.item() if z_s.numel() == 1 else z_s[0].item()
    else:
        z_s_np = z_s

    # Get H0 value
    if isinstance(H0, torch.Tensor):
        H0_np = H0.item() if H0.numel() == 1 else H0[0].item()
    else:
        H0_np = H0 if H0 else 70.0

    D_l = angular_diameter_distance(z_l_np, H0_np, Omega_m)  # Mpc
    D_s = angular_diameter_distance(z_s_np, H0_np, Omega_m)  # Mpc
    D_ls = angular_diameter_distance(
        z_s_np, H0_np, Omega_m
    ) - angular_diameter_distance(z_l_np, H0_np, Omega_m) * (1 + z_l_np) / (
        1 + z_s_np
    )  # Approximate

    # For proper D_ls, use: D_ls = D_s * (1+z_l)/(1+z_s) - D_l approximately
    D_ls = D_s * (1 + z_l_np) / (1 + z_s_np) - D_l

    # Convert to kpc
    D_l = D_l * 1000  # kpc
    D_s = D_s * 1000  # kpc
    D_ls = max(D_ls * 1000, 1.0)  # kpc, ensure positive
    ...
    if mask_less.any():
        ...
        arctanh_term = torch.atanh(
            sqrt_term.clamp(-0.999, 0.999)
        )  # Clamp for stability
        f_x[mask_less] = (
            1.0 - 2.0 * arctanh_term / torch.sqrt(1.0 - x_less**2 + 1e-10)
        ) / (x_less**2 - 1.0)
    ...
    if mask_greater.any():
        ...
        f_x[mask_greater] = (
            1.0 - 2.0 * arctan_term / torch.sqrt(x_greater**2 - 1.0 + 1e-10)
        ) / (x_greater**2 - 1.0)
    ...
    Sigma_crit = (c_kpc**2 / (4.0 * np.pi * G)) * (
        D_s / (D_l * D_ls + 1e-8)
    )  # M_sun/kpc²
...
def physics_informed_loss(
    ...
    device: str = "cpu",
    z_lens: float = None,  # REQUIRED - lens redshift
    z_source: float = None,  # REQUIRED - source redshift
    Omega_m: float = None,  # REQUIRED - matter density
    ...
    z_lens : float, REQUIRED
        Lens redshift - MUST be provided for scientifically valid calculations
    z_source : float, REQUIRED
        Source redshift - MUST be provided for scientifically valid calculations
    Omega_m : float, REQUIRED
        Matter density parameter - MUST be provided for scientifically valid calculations
    ...
    Raises
    ------
    ValueError: If z_lens, z_source, or Omega_m are not provided
    ...
    """
    # CRITICAL: Require cosmological parameters for scientifically valid calculations
    if z_lens is None or z_source is None or Omega_m is None:
        raise ValueError(
            "z_lens, z_source, and Omega_m are REQUIRED parameters. "
            "Cannot use default/hardcoded cosmological parameters. "
            "Please provide actual lens and source redshifts from observational data."
        )
    ...
    H0 = pred_params[:, 4].unsqueeze(1)  # Hubble constant (km/s/Mpc)
    ...
    H0 = torch.clamp(H0, min=40.0, max=100.0)  # [40, 100] km/s/Mpc (physical range)

    # Compute NFW deflection angles using real physics with provided cosmological parameters
    # Process each sample in the batch with its own H0 value
    alpha_x_list = []
    alpha_y_list = []

    for i in range(batch_size):
        H0_i = H0[i, 0].item() if torch.is_tensor(H0) else H0
        alpha_x_i, alpha_y_i = compute_nfw_deflection(
            M_vir[i, 0],
            r_s[i, 0],
            theta_x[i],
            theta_y[i],
        ...
        alpha_x_list.append(alpha_x_i)
        alpha_y_list.append(alpha_y_i)

    alpha_x = torch.stack(alpha_x_list)
    alpha_y = torch.stack(alpha_y_list)
